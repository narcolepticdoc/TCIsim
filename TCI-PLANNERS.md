...
**Trigger = a genuine reversal, detected + backtracked (v0.5.44.7).** Descending on the current tier, the loop refines the moment the tier-snapped rate would *strictly rise* above the last emitted rate (`qRateDiv(exact, TIER_DIV[tier]) > lastRate`) — the tier has bottomed out and is about to flip-flop. It then increments `tier` and **backtracks one step**: restores the engine state / scheme / time saved before the over-descended step (`backState`/`backLen`/`backT`) and re-emits on the finer grid. Repeats (round-to-nearest flats — e.g. a still-declining 68 that rounds up to 70) do NOT trigger; only a strict up-move does, so the plan never emits a reversal at any tier. Keying off the reversal — not the per-step decline (the original `FINE_TRIGGER_FRAC` trigger engaged the fine grid at V3 ~20–32% because the extend-loop makes early steps short) and not an `ssRate` estimate — is fully adaptive with no tuning constant, and keeps the base grid running through the whole clinically active window (a Ce 3.5–5 case is ~93–100% on the 5-grid across 3–8 h; Ce 2–3 stay on integers, 5s then 1s). Backtrack is required: without it the over-step causes a Ce dip and a real 150→155-style rate bounce. Some cases legitimately never refine (the coarse grid holds — ideal). Costs — a rougher band-riding far tail (~0.09 µg/mL) and a ~−2% transition dip — fall at 10–16 h, outside the clinical window. The terminal SS-rate append snaps on the finest tier the loop reached; a target change replans a fresh coarse descent (self-re-arming). Helpers: `qRateDiv` in `js/sim/tci/shared.js`, `rateGridStepMgMin`, and `quantizeInDisplay`'s `stepOverride` param in `js/util/units.js`. Inert when rounding is off (stays on tier 0 = un-quantized). Regression coverage: `tests/test-tci-scheme.mjs` TEST 15 (zero base-grid reversals; Ce converges within the band-riding envelope; target-change re-arm) and TEST 16 (active phase stays on the base grid; finer tiers only later).
