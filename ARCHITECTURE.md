...
- **Curation** is `collectUpcoming()` in `js/sim/upcoming.js` — pure, DOM-free and unit-tested (`tests/test-upcoming.mjs`). It selects and orders items and returns raw event objects; all label formatting stays in the view via `js/util/event-label.js`.
  ...
  - **Latched crossings** (`latched: true`): a redose threshold that has *already* been crossed. Kept as `elapsed` with its own budget — exempt from the lookback and from `maxElapsed` — until a tap or a bolus at/after the crossing clears it. `approach.js` reports `belowThreshold`; `settings.getBelowSince(drugId)` stamps the moment, off the same edge that fires the redose chime.
- **Clock sizing** is fit-to-width: `next-up.js` publishes the glyph count as `data-len`, CSS holds the per-length ceiling in `--nu-fit`, and the text-size tiers cap it. See the width-budget comment above `.nu-clock` in `index.html` before changing any factor.
- **Row times** obey a CD / ET / RT toggle (countdown / case time / real time) (`#btn-nu-time`, persisted under `tci-pref-nextup-time-mode` and in `prefsManifest()`); the clock is always a countdown.
