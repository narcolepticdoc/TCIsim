...
for (const file of files) {
  ...
  const crashed = result.status !== 0 && !match;

  totalPassed += passed;
  totalFailed += failed + (crashed ? 1 : 0);

  const label = file.replace(/^test-/, '').replace(/\.js$/, '');
  const status = crashed ? '  CRASH' : failed > 0 ? `  FAIL (${failed})` : '     ok';
  const counts = crashed ? '' : `  ${passed}p / ${failed}f`;
