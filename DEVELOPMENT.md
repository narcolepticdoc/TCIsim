...
Template: per-drug value+unit pairs (display units, so mcg/kg scales with the patient) edited on each drug's setup tab, persisted to `tci-dose-template` on every input; `tci-dose-template-armed` backs the "Give starting doses on Start" checkbox. `buildTemplateDoses` (pure, tested) expands the template into ordered items — per drug, rate before bolus, mirroring the keypad flow so the manual bolus's system rate-restore resumes the template rate; bolus delivery mode follows `pumpEnabled` (push when off); rates with pump off error `rate-needs-pump`; conversion failures (e.g. missing weight) error `conversion-failed`. `applyStartingDoses()` in app.js runs from `onCaseStart` with three guards against double-dosing: `opts.restored` (covers local restore AND cloud-pulled cases — both funnel through `ensureStarted({restored:true})`, and a started case can never re-fire `onCaseStart`), armed+non-empty, and per-drug "already has events" (pre-start TCI plans / planned boluses win over the template). Skipped/errored items surface in the "Starting Doses Given" notation sub-line and console, never blocking case start.
