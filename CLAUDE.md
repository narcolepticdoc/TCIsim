...
Mobile-first PWA for anesthesia training. Simulates propofol (Eleveld 2018), fentanyl (Shafer 1990 + Shibutani 2004), and ketamine (Domino 1982 / Navarrete 2000) pharmacokinetics with Target Controlled Infusion (TCI) planning. Current version: **0.5.24.23** (see `js/version.js`).
...
- **Before pushing follow-up commits to an existing branch, check whether the prior PR is merged or closed.** Use `mcp__github__list_pull_requests` (filter by `head: narcolepticdoc:<branch>`) or `pull_request_read` on the known PR number. If the prior PR for this branch is `merged` or `closed`, do NOT assume a `git push` will update it — open a **new PR** for the follow-up commits. Pushing to a branch whose PR is already merged orphans the new commits behind a closed PR.
