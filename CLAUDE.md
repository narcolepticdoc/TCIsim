...
- **Entry buffers format with `formatValueEntry`, readouts with `formatValue`.** `formatValue`'s per-unit decimal cap is blind to each drug's `quantSteps` grid, and where the grid is finer it rounds away precision the user could have entered (a fentanyl redose threshold of 0.35 ng/mL echoed back as 0.3, and confirming the prefill moved it). `formatValueEntry` takes the *finer* of cap and grid, so it can neither truncate nor — as `formatValueStep` alone would — coarsen a 22.5 mg propofol bolus to 23, and it never renders a positive value as `0`. Every keypad/event-editor prefill and `convertBufferUnit` uses it; drug cards, history and notations keep `formatValue`; planning-mode titration (`setCanonical`) keeps `formatValueStep`, where snapping to the grid is the point.
- **In-case keypad prefills share one path.** `keypad.js prefillFromCanonical(canonicalValue, task)` serves Ce target, redose threshold, emergence, last rate and last bolus — all five sources are canonical. Do not re-inline a `fromCanonical` + format block per type; that is how the `ceTarget` prefill drifted into reading raw canonical with `.toString()`.
- **The emergence countdown's "idle" is a direction, not a rate.** `exit-readout.js` ticks the countdown down 1 s/s and re-predicts only every 5 s while idle, which is valid *only* while Ce is falling. `getRateAtTime` walks rate/pause events and cannot see boluses, so a zero rate says nothing about a bolus that has just gone in: `isIdle` combines `rate === 0` with `!rising` and `!isInBolusPhase` (the latter covers the first seconds, where the plasma is loaded but the effect site is still coasting down). Never simplify it back to the pump rate — that counted the wrong way for the whole post-bolus rise, by 76 s on propofol and ~52 min on a ketamine push.
- **`predictTroughTime`'s bisection tolerance is relative to the target, never absolute.** An absolute µg/mL band is 0.01% of a propofol emergence target and 33% of a fentanyl one; at that width bisection returns on its first probe and the answer collapses onto the 0.5 min coarse grid. This function backs both the emergence and redose countdowns for every drug.
...
- **The pump horizon may hide a drug's later steps, never its next one.** `guaranteeNextPerDrug` admits each drug's soonest future pump event however distant — once, and only when the horizon walk left that drug with no row. The horizon is a *density* control (a dense TCI plan), and late in a long case the planner's maintenance steps spread out past 20 min apart, so without the floor the panel goes silent about the only action that drug has scheduled. Do not "simplify" it into a wider `horizonMin` — that re-breaks the dense early-case plan the 20 min budget exists for.
...
- **The chart stack is vendored, never CDN-loaded.** Chart.js and its annotation/zoom/hammer plugins live in `js/vendor/` and are listed in `sw.js`'s *mandatory* same-origin precache. This is not a preference: a missing annotation plugin does not throw — Chart.js draws the curves and silently discards every annotation, so the threshold lines and nomogram bands vanish with an empty console. `createChart` asserts both plugins registered (`assertChartPlugins`). Never move these to a CDN, and never move them into the tolerant precache group.
...
- `tci_lastBolus_{drug}` / `tci_lastRate_{drug}` — last dose given, in canonical mg / mg/min; prefills the bolus and rate keypads. **Cleared when a case begins**, by `session.js resetCaseCarryOver()` — which also reseeds the working unit keys, and must be called from *both* `newCase()` and setup's `onConfirm`: a cold start (page reload, PWA relaunch, or the reload after a service-worker update) reaches the setup screen without `newCase()` ever running. A canonical dose re-derives against the new patient's weight in a per-kg display unit, so carrying it across cases showed the previous patient's dose as a different mg/kg. Cross-case doses belong in the starting-dose template; `restore()` deliberately keeps the memory, since it resumes the same case.
