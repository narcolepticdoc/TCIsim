...
- `tci_lastBolus_{drug}` / `tci_lastRate_{drug}` — last dose given, in canonical mg / mg/min; prefills the bolus and rate keypads. **Cleared on New Case** (`session.js newCase()`, same `DRUG_IDS` loop that reseeds the working units): a canonical dose re-derives against the new patient's weight in a per-kg display unit, so carrying it across cases showed the previous patient's dose as a different mg/kg. Cross-case doses belong in the starting-dose template.
