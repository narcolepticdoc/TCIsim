...
**Session 21 (2026-04-07):** TCI target popup with delay selector and first-step countdown. Version 0.5.7.

When setting a TCI target during a running case, `planTCI` is no longer called immediately on keypad confirm. Instead, a delay selector modal appears showing the Ce target and pill buttons for 5/10/15/20/30s (default 10s, selection persists within session). On confirm, `planTCI` is called with `futureTime = timer.getElapsedMinutes() + delaySeconds / 60` so the plan starts in the future. A countdown modal then appears immediately: the first step of the TCI scheme is described in plain terms ("Bolus 280 mcg/kg = 4.0 mL" or "Set rate to 30 mL/hr" or "Hold infusion (pump off)") with a large amber ticker counting down from the selected delay. Ticker updates every 100 ms; at zero it shows "Now!" and auto-dismisses after 1.5 s. "Got it" button dismisses early. Pre-case TCI targeting (before Start is pressed) is unchanged — plan commits immediately with no delay modal. Implementation: two new HTML modals (`#modal-tci-delay`, `#modal-tci-firststep`) with supporting CSS pill and countdown styles added to `index.html`; `app.js` imports `getPumpSettings`, stores `pendingTCI`/`tciDelaySeconds`/`tciCountdownInterval` state, intercepts `type==='ceTarget'` confirm during running case with early `return`, and adds `showTciDelayModal()`, `commitTciDelay()`, and `showTciFirstStepModal()` functions. Escape and click-outside handlers extended to clear pending state and cancel countdown. `simulation.js` unchanged — it already returned `{ scheme, events }` from `planTCI`. 359 tests across 12 suites, all passing. Branch: `claude/tci-target-popup-a9rWz`.
